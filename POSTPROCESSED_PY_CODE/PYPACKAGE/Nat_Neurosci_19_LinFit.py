""" Ref) Chaudhuri. et al. Nat. Neursci. 2019 """
""" (Edit) June 3rd 2022: Jiho Lee, PhD Candidate, IMS Lab. """
...
from PYPACKAGE import Nat_Neurosci_19_FitHelpFns as fhf
from PYPACKAGE import Nat_Neurosci_19_AngFns as af
...
class PiecewiseLinearFit:
    ...
    def fit_data(self, fit_params):
        ...
        def cost_fn(flat_knots, penalty_params = fit_params):
            ...
            # Get the distance between a point of 'data_to_fit point' and its closest point at 'fit_curve'
        ...
        # cost_test = cost_fn(flat_init_knots)
...
def fit_manifold(data_to_fit, fit_params):
    ...
    """ Custom code by Jiho Lee (2022) """
    order = fit_params['order']
    fit_results['final_knots'] = knots_along_time(fitter.saved_knots[0]['knots'], data_to_fit, order)
    fit_results['loop_final_knots'] = fhf.loop_knots(fit_results['final_knots'])

    
    fit_results['tt'], fit_results['curve'] = fhf.get_curve_from_knots(
        fit_results['loop_final_knots'], 'eq_vel', dt = fit_params['dalpha'])

    fit_results['tt'], fit_results['curve'] = roll_manifold_to_fit_ref(
        fit_results['tt'],fit_results['curve'], data_to_fit, order)
        

    return fit_results

""" Custom code by Jiho Lee (2022) """
def match_mani_to_ref_bphr(mani_pts, mani_coords, data_to_fit, ref_bphr):
    neigh = NearestNeighbors(n_neighbors = 10).fit(data_to_fit)
    _, idx = neigh.kneighbors(mani_pts)
    idx_of_nearst_data_pts = np.squeeze(idx)
    matched_bphr = np.mean(ref_bphr[idx_of_nearst_data_pts], axis = 1)
    # matched_bphr = ref_bphr[idx_of_nearst_data_pts]
    return matched_bphr

def knots_along_time(knot_pts, data_to_fit, ref):
    # determine to flip the knots or not
    neigh = NearestNeighbors(n_neighbors = 3).fit(data_to_fit)
    _, idx = neigh.kneighbors(knot_pts)
    _, idx_f = neigh.kneighbors(np.flip(knot_pts, axis = 0))
    idx = np.squeeze(idx)
    idx_f = np.squeeze(idx_f)
    score = np.mean(ref[idx], axis = 1)
    score_f = np.mean(ref[idx_f], axis = 1)
    # score = score[1:] - score[0:-1]
    # score_f = score_f[1:] - score_f[0:-1]
    score =  max(np.correlate(np.arange(len(idx)), np.hstack((score[1:], score)), mode='valid'))
    score_f = max(np.correlate(np.arange(len(idx)), np.hstack((score_f[1:], score_f)), mode='valid'))
    if score < score_f:
        final_knot = np.flip(knot_pts, axis = 0)
    else:
        final_knot = knot_pts
    return final_knot

def roll_manifold_to_fit_ref(mani_local_coords, mani_curve_pts, data_to_fit, ref):
    neigh = NearestNeighbors(n_neighbors = 3).fit(data_to_fit)
    _, idx = neigh.kneighbors(mani_curve_pts)
    idx = np.squeeze(idx)
    match = np.mean(ref[idx], axis = 1)
    rolling_lag = np.argmax(np.correlate(np.arange(len(match)), np.hstack((match[1:], match)), mode='valid'))
    
    rolled_pts = np.roll(mani_curve_pts, rolling_lag, axis = 0)
    rolled_crd = np.roll(mani_local_coords, rolling_lag, axis = 0)

    return rolled_crd, rolled_pts

# def decode_from_passed_fit(data_to_decode, fit_coords, fit_curve, ref_angles):
def decode_from_passed_fit(data_to_decode, fit_coords, fit_curve, fit_bphr):
    ...
    #     ref_angles)
    _,_, indices_of_nearst_mani_pts = fhf.get_closest_manifold_coords(fit_curve, 
        fit_coords, data_to_decode, return_all = True)

    dec_bphr = fit_bphr[indices_of_nearst_mani_pts]
